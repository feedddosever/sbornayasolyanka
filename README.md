...
*Mission 03 (Live wire).* The bid book updates from an Arkiv **websocket subscription**, wired into the market page — there is no network timer anywhere in it. `src/arkiv/watch.ts` builds the client on a `webSocket` transport and passes no `fromBlock`; both details matter, because the documented example uses `http()` and the page itself says it polls, so following it literally builds the loop the mission excludes. Events are filtered at the handler to the three types that can change the book. A burst of events is debounced into one query.

Reconnection is the interesting part: you cannot both replay a gap and stay on a socket, because `fromBlock` forces the polling path. So the stream carries *"something changed"* and the compound query carries *"here is the truth"* — a dropped connection costs freshness for a moment, never correctness. The UI shows the live socket state, and the countdown between events is interpolated locally from the `expiresAt` block height the node already returned, which is a render timer and not a poll.
